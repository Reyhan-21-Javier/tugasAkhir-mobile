...
const LocationScreen = () => {
  ...
  const handleUseGPS = async () => {
    if (location) {
      ...
      // Tampilkan teks "Lokasi Anda" saat data lokasi diperbarui
      Alert.alert(
        'Lokasi Anda',
        `Lokasi Anda: ${address[0].city}, ${address[0].region}`,
        [
          { text: 'OK', onPress: () => navigation.navigate('HomePage', { selectedKecamatan: nearestKecamatan.name }) }
        ]
      );
    }
  };   
  ...
  const handleSelect = () => {
    if (!selectedKecamatan) {
      Alert.alert('Peringatan', 'Silahkan pilih kecamatan terlebih dahulu');
      return;
    }
    navigation.navigate('HomePage', { selectedKecamatan });
  };

  return (
    <View style={styles.container}>
      <Image source={require('../assets/maps.png')} style={styles.image} />
      <Text style={styles.heading}>Silahkan pilih lokasi anda atau{'\n'}menggunakan mode GPS</Text>
      <Picker
        selectedValue={selectedKecamatan}
        style={[styles.inputContainer, styles.picker]}
        onValueChange={(itemValue, itemIndex) => setSelectedKecamatan(itemValue)}
      >
        <Picker.Item label="Cari Kecamatan..." value="" />
        {kecamatanBogor.map((kecamatan, index) => (
      ...
      <TouchableOpacity style={styles.gpsButton} onPress={handleUseGPS}>
        <Text style={styles.gpsButtonText}>Pakai GPS</Text>
      </TouchableOpacity>
    </View>
  );
};
